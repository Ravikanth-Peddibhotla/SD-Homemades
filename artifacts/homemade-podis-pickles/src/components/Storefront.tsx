...
export function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return <article data-testid={`card-product-${product.id}`} className={`group ${compact ? 'w-[172px] shrink-0' : ''}`}>
    <div className="relative overflow-hidden rounded-[22px] border border-[#e2d1bc] bg-[#ead8b8]" style={{ aspectRatio: compact ? '1 / 1.08' : '1 / 1.05' }}>
      ...
      <span className="absolute left-3 top-3 rounded-full bg-[#fffaf2]/90 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.1em] text-[#6d2925]">{product.offer?.name ?? product.tags[0]}</span>
    ...
    <div className="mt-1 flex items-center justify-between"><span className="text-[13px] font-bold">{formatPrice(product.price)}{product.offer && <del className="ml-1 text-[10px] font-normal text-[#9b897a]">{formatPrice(product.compareAt ?? product.price)}</del>}</span><span className="flex items-center gap-1 text-[10px] font-semibold text-[#806b5b]"><Star size={11} className="text-[#a9472f]" fill="currentColor" />{product.rating}</span></div>
  </article>;
}
